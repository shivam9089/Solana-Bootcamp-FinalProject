...
/// 2. `[writable]` treasure: [TreasureMetadata] 
...
/// - short_description: [String] 
	pub fn process_mint(
		...
		let treasure_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (treasure_pubkey, treasure_bump) = Pubkey::find_program_address(
			&[b"treasure", mint_info.key.as_ref()],
			program_id,
		);
		...
		if *treasure_info.key != treasure_pubkey {
		...
		let space = TreasureMetadata::LEN;
		let rent = Rent::get()?;
		let rent_minimum_balance = rent.minimum_balance(space);
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), treasure_info.clone()],
			&[&[b"treasure", mint_info.key.as_ref(), &[treasure_bump]]],
		)?;
		...
		if treasure_info.data_len() != TreasureMetadata::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let treasure = &mut AccountPDA::new(
			&treasure_info,
			try_from_slice_unchecked::<TreasureMetadata>(&treasure_info.data.borrow()).unwrap(),
			treasure_bump,
		);
		...
		treasure.data.serialize(&mut &mut treasure_info.data.borrow_mut()[..])?;
