...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let frequency: String = Default::default();
		...
		let (treasure_pubkey, _) = Pubkey::find_program_address(
			&[b"treasure", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(treasure_pubkey, false),
